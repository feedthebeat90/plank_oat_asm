...
handcoded <- read.csv("handcoded.csv")
head(handcoded)
...
# Concatenating every row of the dataframe - We know every concat-string here is a Positive Match (1)
handcoded_vec = paste(handcoded$amicus, handcoded$bonica, sep="_")
handcoded_vec

tmp = read.csv("handcoded.csv")
# I am commenting the line below because temp is supposed to contain handcoded.csv
# tmp = handcoded

# Shuffling amicus column
tmp$amicus = sample(tmp$amicus, replace=F)
# Apparently after sampling from amicus column from temp we are also slicing that column from tmb original dataframe
tmp
# He reconcat the shuffled amicus column with the original bonica column with an underscore separator
temp_vec = paste(tmp$amicus, tmp$bonica, sep="_")
temp_vec
# Finally he will check these new shuffled concatenations with the original concatenations
tmp = tmp[!temp_vec %in% handcoded_vec,]
# Notice that a 0 (negative match) will be assigned each time the shuffled concatenation don t match with the original concatenations
tmp$match = 0
tmp
nrow(tmp)
ncol(tmp)

tmp2 = read.csv("handcoded.csv")
...
tmp_full
colnames(tmp_full) = c("X","amicus", "bonica", "match")
tmp_full
ncol(tmp_full)
nrow(tmp_full)
